// ======================================================
// // results.js — 시험결과 입력/판정 (차체·휠·DGU 내부 분기)
// 의존: common.js
// ======================================================
...
function rsInit(){
  if(CUR_SYS === 'dgu') { dguRsInit(); return; }
  // 차체/휠 공통 (activeDB()로 자동 분리)
...
// ══════════════════════════════════════════════════════
// 휠도장 시험결과 — rsInit() 재사용 (activeDB()로 자동 분리)
// 휠 전용 판정 기준
// ══════════════════════════════════════════════════════
const RS_SPEC_WHEEL = {
  '부착성':     {judge:(d)=> d?.grade && adhesionNum(d.grade)<=2.5 ? 'pass':'fail'},
  '내수성':     {judge:(d)=> d?.appearance==='이상없음' && d?.adhesion && adhesionNum(d.adhesion)<=2.5 ? 'pass':'fail'},
  '내습성':     {judge:(d)=> d?.appearance==='이상없음' && d?.adhesion && adhesionNum(d.adhesion)<=2.5 ? 'pass':'fail'},
  '내치핑성':   {judge:(d)=> d?.grade && parseFloat(d.grade)<=3 ? 'pass':'fail'},
  '내염수분무성':{judge:(d)=> d?.peeling && parseFloat(d.peeling)<=3 ? 'pass':'fail'},
  'CASS시험':   {judge:(d)=> d?.result==='이상없음' ? 'pass':'fail'},
};

// ══════════════════════════════════════════════════════
// DGU 시험결과 — 전용 UI (피착제 × 조건 매트릭스)
// ══════════════════════════════════════════════════════
const DGU_CONDITIONS_LIST = ['상태','내열','내수','내약품','내열CYCLE'];

const RS_SPEC_DGU = {
  'KNIFE CUT':  {judge:(cf)=> cf>=90 ? 'pass':'fail', unit:'%', label:'Cf(%)'},
  '전단강도':   {judge:(v)=>  v>=3   ? 'pass':'fail', unit:'MPa'},
  '경도':       {judge:(v)=>  v>=45 && v<=70 ? 'pass':'fail', unit:'Hs'},
  '인장강도':   {judge:()=>'pass', unit:'MPa', ref:true},
  '신율':       {judge:()=>'pass', unit:'%',   ref:true},
  '인열강도':   {judge:(v)=> v>=15  ? 'pass':'fail', unit:'N/mm'},
};
...
function dguRsInit(){
  ...
  if(!sel) return;
  sel.innerHTML = '';
  Object.keys(DB_DGU.orders).sort().reverse().forEach(yr=>{
    const opt=document.createElement('option');
    opt.value=yr; opt.textContent=yr+'년';
    if(yr===CY) opt.selected=true;
    sel.appendChild(opt);
...
function dguRsChangeYear(yr){
  const sel = document.getElementById('rs-order-sel');
  if(!sel) return;
  sel.innerHTML = '<option value="">-- 차수 선택 --</option>';
  (DB_DGU.orders[yr]||[]).forEach(o=>{
    const opt=document.createElement('option');
    opt.value=o.id;
    opt.textContent=`${o.cha} — ${(o.purpose||'').slice(0,20)}`;
    sel.appendChild(opt);
  });
  document.getElementById('rs-list').innerHTML = '<div style="text-align:center;padding:40px;color:var(--tx3)">차수를 선택하세요</div>';
  document.getElementById('rs-detail').innerHTML = '';
}

function dguRsSelectOrder(orderId){
  if(!orderId) return;
  const yr  = document.getElementById('rs-year-sel').value;
  const order = (DB_DGU.orders[yr]||[]).find(o=>o.id===orderId);
  if(!order) return;
  if(!order.rsData) order.rsData = {};
  dguRsRenderList(order);
}

function dguRsRenderList(order){
  // 피착제 목록 (공통/실러 제외, 실제 피착제만)
  const sps = order.specimens.filter(sp=>!sp.maker.startsWith('['));
  const list = document.getElementById('rs-list');
  list.innerHTML = sps.map((sp,i)=>`
    <div onclick="dguRsSelectSpec('${order.id}','${sp.maker}','${sp.color}')"
      style="padding:10px 12px;border-bottom:1px solid var(--border);cursor:pointer;transition:.15s"
      onmouseover="this.style.background='var(--bg3)'" onmouseout="this.style.background=''">
      <div style="font-size:11px;color:var(--tx3)">${sp.maker}</div>
      <div style="font-size:13px;font-weight:700;font-family:var(--mono)">${sp.color}</div>
    </div>`).join('');
  // 실러물성 행도 추가
  const sealer = order.specimens.find(sp=>sp.maker==='[실러업체]');
  if(sealer){
    list.innerHTML += `
      <div onclick="dguRsSelectSpec('${order.id}','[실러업체]','${sealer.color}')"
        style="padding:10px 12px;border-bottom:1px solid var(--border);cursor:pointer;background:var(--obg)"
        onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">
        <div style="font-size:11px;color:var(--o)">실러물성</div>
        <div style="font-size:12px;font-weight:700;color:var(--o)">${sealer.color}</div>
      </div>`;
  }
}

function dguRsSelectSpec(orderId, maker, color){
  const yr = document.getElementById('rs-year-sel').value;
  const order = (DB_DGU.orders[yr]||[]).find(o=>o.id===orderId);
  if(!order) return;
  if(!order.rsData) order.rsData = {};
  ...
  // 실러물성 전용 UI
  if(maker==='[실러업체]'){
    detail.innerHTML = dguRsRenderSealer(order, key, sp);
    return;
  }

  // KNIFE CUT / 전단강도 / 경도 — 조건별 3회 입력 매트릭스
  const secNames = sp.sections.map(s=>s.name);
  let html = `<div style="padding:16px">
    <div style="font-size:13px;font-weight:700;margin-bottom:16px;font-family:var(--mono)">${maker} · ${color}</div>`;

  secNames.forEach(secName=>{
    const conditions = secName==='경도'
      ? ['상태','내열','내수']
      : DGU_CONDITIONS_LIST;
    if(!order.rsData[key][secName]) order.rsData[key][secName] = {};

    html += `<div style="margin-bottom:20px">
      <div style="font-size:12px;font-weight:700;color:var(--tx2);margin-bottom:8px;padding:6px 10px;background:var(--bg3);border-radius:6px">${secName}</div>
      <div style="overflow-x:auto">
      <table style="width:100%;border-collapse:collapse;font-size:12px">
        <thead><tr style="background:var(--bg4)">
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:left">조건</th>
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">1회</th>
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">2회</th>
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">3회</th>
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">평균</th>
          ${secName==='KNIFE CUT'?'<th style="padding:6px 10px;border:1px solid var(--border);text-align:center">L0</th><th style="padding:6px 10px;border:1px solid var(--border);text-align:center">L1</th>':''}
          <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">판정</th>
        </tr></thead>
        <tbody>`;

    conditions.forEach(cond=>{
      if(!order.rsData[key][secName][cond]) order.rsData[key][secName][cond] = {};
      const d = order.rsData[key][secName][cond];
      const vals = [d.v1||'', d.v2||'', d.v3||''];
      ...
      // 판정
      let judge = '';
      const spec = RS_SPEC_DGU[secName];
      if(spec && avg!==null){
        const cfAvg = secName==='KNIFE CUT' ? avg : avg;
        judge = spec.judge(cfAvg) === 'pass'
          ? '<span style="color:var(--g);font-weight:700">합격</span>'
          : '<span style="color:var(--r);font-weight:700">불합격</span>';
      }

      const inpStyle = 'width:60px;padding:3px 6px;font-size:12px;font-family:var(--mono);background:var(--bg3);border:1px solid var(--border);border-radius:4px;text-align:center;outline:none';
      const knifeExtra = secName==='KNIFE CUT' ? `
        <td style="padding:4px 8px;border:1px solid var(--border);text-align:center">
          <input type="number" value="${d.l0||''}" placeholder="L0" style="${inpStyle}"
            onchange="dguRsUpdateVal('${orderId}','${key}','${secName}','${cond}','l0',this.value);dguRsRefreshRow(this)">
        </td>
        <td style="padding:4px 8px;border:1px solid var(--border);text-align:center">
          <input type="number" value="${d.l1||''}" placeholder="L1" style="${inpStyle}"
            onchange="dguRsUpdateVal('${orderId}','${key}','${secName}','${cond}','l1',this.value);dguRsRefreshRow(this)">
        </td>` : '';

      html += `<tr>
        <td style="padding:6px 10px;border:1px solid var(--border);font-weight:600">${cond}</td>
        ${[1,2,3].map(i=>`<td style="padding:4px 8px;border:1px solid var(--border);text-align:center">
          <input type="number" value="${d['v'+i]||''}" placeholder="-" style="${inpStyle}"
            onchange="dguRsUpdateVal('${orderId}','${key}','${secName}','${cond}','v${i}',this.value);dguRsRefreshRow(this)">
        </td>`).join('')}
        <td style="padding:6px 10px;border:1px solid var(--border);text-align:center;font-family:var(--mono);font-weight:700" id="dgu-avg-${orderId}-${key}-${secName}-${cond}">
          ${avg!==null ? avg.toFixed(2) : '—'}
        </td>
        ${knifeExtra}
        <td style="padding:6px 10px;border:1px solid var(--border);text-align:center" id="dgu-judge-${orderId}-${key}-${secName}-${cond}">
          ${judge || '—'}
        </td>
      </tr>`;
    });
  ...
  html += `<div style="display:flex;gap:8px;justify-content:flex-end;margin-top:12px">
    <button class="btn primary" onclick="saveAll()">저장</button>
  </div></div>`;
  detail.innerHTML = html;
}

function dguRsRenderSealer(order, key, sp){
  if(!order.rsData[key]) order.rsData[key] = {};
  const items = sp.sections[0]?.items || [];
  let html = `<div style="padding:16px">
    <div style="font-size:13px;font-weight:700;margin-bottom:16px">실러물성</div>
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      <thead><tr style="background:var(--bg4)">
        <th style="padding:6px 10px;border:1px solid var(--border)">항목</th>
        <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">1회</th>
        <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">2회</th>
        <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">3회</th>
        <th style="padding:6px 10px;border:1px solid var(--border);text-align:center">평균</th>
  ...
  if(!order.rsData[key][secName]) order.rsData[key][secName] = {};
  items.forEach(item=>{
    const d = order.rsData[key][secName][item.name] || {};
    const vals = [d.v1,d.v2,d.v3].map(v=>parseFloat(v)).filter(v=>!isNaN(v));
    const avg = vals.length ? (vals.reduce((a,b)=>a+b,0)/vals.length) : null;
    const spec = RS_SPEC_DGU[item.name];
    const judge = spec && avg!==null
      ? (spec.judge(avg)==='pass'?'<span style="color:var(--g);font-weight:700">합격</span>':'<span style="color:var(--r);font-weight:700">불합격</span>')
      : (spec?.ref ? '<span style="color:var(--tx3)">참고</span>' : '—');
    const inpStyle = 'width:60px;padding:3px 6px;font-size:12px;font-family:var(--mono);background:var(--bg3);border:1px solid var(--border);border-radius:4px;text-align:center;outline:none';
    html += `<tr>
      <td style="padding:6px 10px;border:1px solid var(--border);font-weight:600">${item.name}</td>
      ${[1,2,3].map(i=>`<td style="padding:4px 8px;border:1px solid var(--border);text-align:center">
        <input type="number" value="${d['v'+i]||''}" placeholder="-" style="${inpStyle}"
          onchange="dguRsUpdateVal('${order.id}','${key}','${secName}','${item.name}','v${i}',this.value)">
      </td>`).join('')}
      <td style="padding:6px 10px;border:1px solid var(--border);text-align:center;font-family:var(--mono);font-weight:700">
        ${avg!==null ? avg.toFixed(2) : '—'}
      </td>
      <td style="padding:6px 10px;border:1px solid var(--border);text-align:center">${judge}</td>
    </tr>`;
  });
  html += `</tbody></table>
...
function dguRsUpdateVal(orderId, key, secName, cond, field, val){
  const yr = document.getElementById('rs-year-sel').value;
  const order = (DB_DGU.orders[yr]||[]).find(o=>o.id===orderId);
  if(!order) return;
  if(!order.rsData) order.rsData = {};
  ...
  if(!order.rsData[key][secName]) order.rsData[key][secName] = {};
  if(!order.rsData[key][secName][cond]) order.rsData[key][secName][cond] = {};
  order.rsData[key][secName][cond][field] = val;
  autoSave();
}

function dguRsRefreshRow(inputEl){
  // 평균/판정 즉시 업데이트 — 재렌더 없이 DOM 직접 업데이트
  const row = inputEl.closest('tr');
  if(!row) return;
  const inputs = [...row.querySelectorAll('input[type=number]')].slice(0,3);
  const vals = inputs.map(i=>parseFloat(i.value)).filter(v=>!isNaN(v));
  const avg = vals.length ? (vals.reduce((a,b)=>a+b,0)/vals.length) : null;
  const avgCell = row.querySelector('[id^="dgu-avg-"]');
