// ======================================================
// // report.js — 보고서 생성·출력 (차체·휠·DGU 내부 분기)
// 의존: common.js
// ======================================================
...
function rpRenderWheel(order){
  ...
  const coverHtml = `<div class="rp-page">
    ...
    <div class="rp-title">휠 도장 평가 결과 보고서</div>
    <div class="rp-subtitle">WHEEL COATING TEST REPORT</div>
    <table class="rp-meta-table">
      <tr><td>접수일자</td><td>${order.date||today}</td></tr>
      <tr><td>발급일자</td><td>${today}</td></tr>
      <tr><td>의 뢰 자</td><td>현대자동차㈜ — ${order.mgr||''} 책임연구원</td></tr>
      ...
      <tr><td>평가목적</td><td>${order.purpose||''}</td></tr>
      <tr><td>평가업체</td><td>${makers.join(', ')}</td></tr>
      <tr><td>평가차종</td><td>${allColors.join(', ')}</td></tr>
      <tr><td>평가항목</td><td>${[...new Set(order.specimens.flatMap(sp=>sp.sections.flatMap(s=>s.items.filter(it=>it.checked).map(it=>it.name))))].join(', ')}</td></tr>
      <tr><td>평가결과</td><td>${rpSummaryResult(order)}</td></tr>
    </table>
    <div class="rp-footer">본 보고서는 FITI시험연구원이 발행한 공식 평가결과 보고서입니다.</div>
  </div>`;
  ...
  makers.forEach(maker=>{
    ...
    const wheelSpecs = {
      '부착성':'M-2.5 이하','내수성':'외관이상없음 / M-2.5 이하',
      '내습성':'외관이상없음 / M-2.5 이하','내치핑성':'3급 이하',
      '내염수분무성':'편측 3mm 미만','CASS시험':'240Hr 균열·박리·부식 없을 것',
      'IR분析':'IR PEAK 위치 표시 그래프 제출','도막두께':'층별 도막두께 사양',
    };
    const wheelMethods = {
      '부착성':'바둑판 눈금법 (Nichiban 테이프)',
      '내수성':'40℃ 순수 120Hr 침적','내습성':'50℃ / RH98% / 120Hr',
      '내치핑성':'Gravelometer (-30℃, 150g, 4bar)',
      '내염수분무성':'SST 720Hr (5% NaCl)',
      'CASS시험':'KS D 9502 CASS 240Hr','IR분析':'FT-IR Spectrometer',
      '도막두께':'밴드쏘+마운팅+폴리싱+광학현미경',
    };

    let tableHtml = `<table class="rp-result-table">
      <thead><tr>
        <th rowspan="2">구분</th><th rowspan="2">항목</th>
        <th rowspan="2">시험방법</th><th rowspan="2">요구사항</th>
        <th colspan="${colors.length}" style="background:#dce8f8">${maker}</th>
      </tr><tr>${colors.map(c=>`<th style="background:#dce8f8">${c}</th>`).join('')}</tr></thead><tbody>`;
    ...
    secNames.forEach(secName=>{
      ...
      items.forEach((itemName,idx)=>{
        const cells=colors.map(color=>{
          ...
          const judge=d?.judge;
          const jClass=judge==='pass'?'rp-pass':judge==='fail'?'rp-fail':'rp-na';
          return `<td><div style="font-size:8.5pt">${d?.value||'—'}</div>${judge?`<div class="${jClass}">${judge==='pass'?'합격':'불합격'}</div>`:''}</td>`;
        }).join('');
        tableHtml+=`<tr>
          ${idx===0?`<td class="left" rowspan="${items.length}" style="font-weight:600;background:#f8f8f8">${secName}</td>`:''}
          <td class="left">${itemName}</td>
          <td style="font-size:8pt">${wheelMethods[itemName]||'—'}</td>
          <td style="font-size:7.5pt">${wheelSpecs[itemName]||'—'}</td>
          ${cells}
        </tr>`;
      });
    });
    ...
    resultPages.push(`<div class="rp-page">
      <div style="display:flex;align-items:baseline;gap:8mm;margin-bottom:3mm;border-bottom:2px solid #000;padding-bottom:2mm">
        <div style="font-size:11pt;font-weight:700">평 가 결 과 (휠도장)</div>
        <div style="font-size:9pt;color:#333">${maker} — ${colors.join(', ')}</div>
        <div style="font-size:9pt;color:#777;margin-left:auto">${order.cha}</div>
      </div>
      ${tableHtml}
      <div class="rp-footer">(재)FITI시험연구원</div>
    </div>`);
  });
...
// DGU 보고서
// ══════════════════════════════════════════════════════
function rpRenderDgu(order){
  const body = document.getElementById('rp-body');
  if(!body) return;
  const today = new Date().toLocaleDateString('ko-KR',{year:'numeric',month:'2-digit',day:'2-digit'});
  const makers = [...new Set(order.specimens.filter(sp=>!sp.maker.startsWith('[')).map(sp=>sp.maker))];

  const dguSpecs = {
    'KNIFE CUT':'Cf 90% 이상 (응집파괴)','전단강도':'3 MPa 이상',
    '경도':'45~70 Hs','인장강도':'참고치','신율':'참고치','인열강도':'15 N/mm 이상',
  };
  const dguMethods = {
    'KNIFE CUT':'KNIFE CUT법 (상태·내열·내수·내약품·내열CYCLE)',
    '전단강도':'UTM 전단시험 (각 조건)','경도':'Shore A (5회 평균)',
    '인장강도':'UTM (3호형 시편)','신율':'UTM 표선거리 20mm','인열강도':'UTM (인열강도 시편)',
  };
  ...
  const coverHtml = `<div class="rp-page">
    ...
    <div class="rp-title">DGU 물성 평가 결과 보고서</div>
    <div class="rp-subtitle">DGU ADHESIVE PERFORMANCE TEST REPORT</div>
    <table class="rp-meta-table">
      <tr><td>접수일자</td><td>${order.date||today}</td></tr>
      <tr><td>발급일자</td><td>${today}</td></tr>
      <tr><td>의 뢰 자</td><td>현대자동차㈜ — ${order.mgr||''} 책임연구원</td></tr>
  ...
  // DGU 결과표: 피착제 × 조건 매트릭스
  const resultPages = [];
  const conditions_kc  = ['상태','내열','내수','내약품','내열CYCLE'];
  const conditions_hard = ['상태','내열','내수'];

  const sps = order.specimens.filter(sp=>!sp.maker.startsWith('['));
  const secNames = [...new Set(sps.flatMap(sp=>sp.sections.map(s=>s.name)))];

  secNames.forEach(secName=>{
    const conditions = secName==='경도' ? conditions_hard : conditions_kc;
    const colorsForSec = sps.filter(sp=>sp.sections.some(s=>s.name===secName)).map(sp=>sp.color);
    if(!colorsForSec.length) return;

    let tableHtml = `<table class="rp-result-table">
      <thead><tr>
        <th>조건</th><th>시험방법</th><th>요구사항</th>
        ${colorsForSec.map(c=>`<th style="background:#dce8f8">${c}</th>`).join('')}
      </tr></thead><tbody>`;

    conditions.forEach(cond=>{
      const cells = colorsForSec.map(color=>{
        const sp = sps.find(s=>s.color===color);
        if(!sp) return '<td>—</td>';
        const key = `${sp.maker}_${color}`;
        const d = order.rsData?.[key]?.[secName]?.[cond];
        const avg = d?.avg || (
          [d?.v1,d?.v2,d?.v3].map(v=>parseFloat(v)).filter(v=>!isNaN(v)).reduce((a,b,_,arr)=>a+b/arr.length,0) || null
        );
        const judge = avg!==null && RS_SPEC_DGU[secName]
          ? (RS_SPEC_DGU[secName].judge(avg)==='pass'?'<div class="rp-pass">합격</div>':'<div class="rp-fail">불합격</div>')
          : '';
        const display = secName==='KNIFE CUT'
          ? (d?.cf!=null?`Cf: ${parseFloat(d.cf).toFixed(1)}%`:'—')
          : (avg!==null ? `${avg.toFixed(2)} ${RS_SPEC_DGU[secName]?.unit||''}` : '—');
        return `<td><div style="font-size:8.5pt">${display}</div>${judge}</td>`;
      }).join('');

      tableHtml += `<tr>
        <td class="left" style="font-weight:600">${cond}</td>
        <td style="font-size:8pt">${dguMethods[secName]||'—'}</td>
        <td style="font-size:7.5pt">${dguSpecs[secName]||'—'}</td>
        ${cells}
      </tr>`;
    });
    ...
    resultPages.push(`<div class="rp-page">
      <div style="display:flex;align-items:baseline;gap:8mm;margin-bottom:3mm;border-bottom:2px solid #000;padding-bottom:2mm">
        <div style="font-size:11pt;font-weight:700">평 가 결 과 — ${secName}</div>
        <div style="font-size:9pt;color:#777;margin-left:auto">${order.cha}</div>
      </div>
      ${tableHtml}
      <div class="rp-footer">(재)FITI시험연구원</div>
...
// ── rpRender 분기 처리
const _rpRenderOrig = rpRender;
function rpRender(order){
  if(CUR_SYS === 'wheel') { rpRenderWheel(order); return; }
  if(CUR_SYS === 'dgu')   { rpRenderDgu(order);   return; }
  _rpRenderOrig(order);
}
